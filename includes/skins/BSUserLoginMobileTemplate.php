 ...
 * BlueSpiceDistribution/MobileFrontend/includes/skins/UserLoginMobileTemplate.php
...
class BSUserLoginMobileTemplate extends UserLoginMobileTemplate {
	...
	public function execute() {
		...
		$stickHTTPS = ( $this->doStickHTTPS() )
			? Html::input( 'wpStickHTTPS', 'true', 'hidden' )
			: '';
		...
		$signupLink = Linker::link( SpecialPage::getTitleFor( 'Userlogin' ),
			...
			[ 'class' => 'mw-mf-create-account mw-ui-block' ], $query );

		$login = Html::openElement( 'div', [ 'id' => 'mw-mf-login', 'class' => 'content' ] );
		...
		if ( isset( $this->data['usedomain'] ) && $this->data['usedomain'] ) {
			...
			$select->addOption( wfMessage( 'bs-distributionconnector-yourdomainname', '' )->plain(), '' );
